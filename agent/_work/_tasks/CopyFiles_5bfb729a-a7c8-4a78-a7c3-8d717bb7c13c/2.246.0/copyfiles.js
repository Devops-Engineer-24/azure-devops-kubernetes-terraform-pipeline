 ...
 */
function stats(path, throwEnoent = true) {
    if (fs.existsSync(path)) {
        return fs.statSync(path);
    }
    else {
        const message = `Entry "${path}" does not exist`;
        if (throwEnoent) {
            tl.warning(message);
...
function filterOutDirectories(paths) {
    return paths.filter((path) => {
        const itemStats = stats(path);
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let matchedFiles = filterOutDirectories(matchedPaths);
        ...
        if (matchedFiles.length > 0) {
            ...
            if (cleanTargetFolder) {
                ...
                const targetFolderStats = yield retryHelper.RunWithRetry(() => stats(targetFolder, false), `stats for ${targetFolder}`);
            ...
            try {
                ...
                for (let file of matchedFiles) {
                    ...
                    if (!cleanTargetFolder) { // optimization - no need to check if relative target exists when CleanTargetFolder=true
                        targetStats = yield retryHelper.RunWithRetry(() => stats(targetPath, false), `Stats for ${targetPath}`);
                    }
                    ...
                    if (!overWrite) {
                        if (targetStats) { // exists, skip
                            ...
                            if (preserveTimestamp) {
                                try {
                                    const fileStats = yield retryHelper.RunWithRetry(() => stats(file), `stats for ${file}`);
